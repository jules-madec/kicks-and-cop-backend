...
class Carts extends Database
{
    ...
    {
        $queryExecute = $this->db->prepare("SELECT COUNT(*) FROM `carts` 
    ...
    public function sizeBelongsToSneaker()
    {
        $queryExecute = $this->db->prepare("SELECT COUNT(*) FROM `sizes` 
            WHERE `id` = :size AND `sneaker` = :sneaker");
    ...
    {
        $queryExecute = $this->db->prepare("DELETE FROM `carts` 
            WHERE `user` = :user AND `sneaker` = :sneaker AND `size` = :size");
    ...
    {
        $queryExecute = $this->db->prepare("SELECT SUM(sneakers.price) 
            FROM carts cart
            JOIN sneakers ON sneakers.id = cart.sneaker
            WHERE cart.user = :user");
        ...
        return (float) $queryExecute->fetchColumn();
    }

    public function countItems()
    {
        $queryExecute = $this->db->prepare("SELECT COUNT(*) FROM `carts` WHERE `user` = :user");
    ...
    {
        $queryExecute = $this->db->prepare("DELETE FROM `carts` WHERE `user` = :user");
